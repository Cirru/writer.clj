...
(defn write-space [tree state]
  (update-in state [:code] (fn [code] (str code " "))))
...
(defn write-string [tree state]
  (update-in
    state
    [:code]
    (fn [code] (str code (with-out-str (clojure.pprint/write tree))))))

(defn write-raw [tree state]
  (update-in state [:code] (fn [code] (str code tree))))
...
(defn write-newline [tree state]
  (update-in
    ...
    [:code]
    (fn [code]
      (str code (string/join "" (repeat (:indentation state) " "))))))
...
(defn write [tree] (:code (write-program initial-state tree)))